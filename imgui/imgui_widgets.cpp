...
bool ImGui::CheckboxSized(const char* label, bool* v, ImVec2 size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + size + style.FramePadding * 2.0f);
    ItemSize(total_bb, style.FramePadding.y);
    const bool is_visible = ItemAdd(total_bb, id);
    const bool is_multi_select = (g.LastItemData.ItemFlags & ImGuiItemFlags_IsMultiSelect) != 0;
    if (!is_visible)
    ...
    if (is_visible)
    {
        ...
        {
            const float pad = ImMax(1.0f, IM_TRUNC(square_sz / 6.0f));
            RenderCheckMark(window->DrawList, pos+size-ImVec2(square_sz, square_sz) + ImVec2(pad, pad), check_col, square_sz - pad * 2.0f);
        }
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, float width)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(total_bb.Min, ImVec2(value_x2, total_bb.Max.y), frame_col, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ImU32 bg_col = frame_col; //GetColorU32((popup_open || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRectFilled(ImVec2(value_x2, total_bb.Min.y), total_bb.Max, bg_col, style.FrameRounding, (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);
        if (value_x2 + arrow_size - style.FramePadding.x <= total_bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, total_bb.Min.y + style.FramePadding.y), GetColorU32(ImGuiCol_CheckMark), ImGuiDir_Down, 1.0f);
    }
    RenderFrameBorder(total_bb.Min, total_bb.Max, style.FrameRounding);

    // Custom preview
    if (flags & ImGuiComboFlags_CustomPreview)
    {
        g.ComboPreviewData.PreviewRect = ImRect(total_bb.Min.x, total_bb.Min.y, value_x2, total_bb.Max.y);
    ...
    return BeginComboPopup(popup_id, total_bb, flags);
}
...
bool ImGui::Combo(const char* label, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count, int popup_max_height_in_items, float width)
{
    ImGuiContext& g = *GImGui;
    ...
    if (!BeginCombo(label, preview_value, width > 0 ? ImGuiComboFlags_WidthFitPreview : ImGuiComboFlags_None, width))
        return false;
...
bool ImGui::Combo(const char* label, int* current_item, const char* const items[], int items_count, int height_in_items, float width)
{
    const bool value_changed = ImGui::Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items, width);
    return value_changed;
}

// Combo box helper allowing to pass all items in a single string literal holding multiple zero-terminated items "item1\0item2\0"
bool ImGui::Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items, float width)
{
    int items_count = 0;
    ...
    bool value_changed = ImGui::Combo(label, current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items, width);
    return value_changed;
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, float width)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = width > 0 ? width : CalcItemWidth();

    const ImVec2 label_size = CalcTextSize(label, NULL, true);

    const float fr_w = width > 0 ? width : label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f;

    const bool isFixed = (flags & ImGuiSliderFlags_FixedWidth) != 0;

    const ImRect frame_bb(window->DC.CursorPos + ImVec2(isFixed?fr_w*0.5f:label_size.x + style.ItemInnerSpacing.x * 2, 0), window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 0.5f));
    const ImRect total_bb(window->DC.CursorPos, frame_bb.Min + ImVec2(fr_w, label_size.y + style.FramePadding.y * 2.0f));
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags, float width)
{
    return ImGui::SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags, width);
}
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags, float width)
{
    return ImGui::SliderScalar(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags, width);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Min.x - ImGui::CalcTextSize(label)[0] - style.ItemInnerSpacing.x*2, frame_bb.Min.y + style.FramePadding.y), label);
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    if (is_visible)
    {
        ...
        bool just_closed;
        bool text_clipped;
        TabItemLabelAndCloseButton(tab_bar, display_draw_list, bb, tab_just_unsaved ? (flags & ~ImGuiTabItemFlags_UnsavedDocument) : flags, tab_bar->FramePadding, label, id, close_button_id, tab_contents_visible, &just_closed, &text_clipped);
